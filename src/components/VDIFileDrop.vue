  ...
  <div>
    <table>
      <tr v-if="files.length">
        <td id="header-row"><b>Files to be uploaded</b></td>
        <td id="header-row">&nbsp;</td>
        <td id="header-row">&nbsp;</td>
      </tr>
      <tr v-for="(file, index) in files" :key="index">
        ...
        <td class="actions-column">
          <button @click.stop="uploadFile(this.selectedViewId, this.selectedViewName, index)" class="upload-button">
      ...
      <tr v-if="files.length > 1">
        ...
        <td class="actions-column" id="footer-row">
          <button @click.stop="uploadFiles(this.selectedViewId, this.selectedViewName, files)" class="upload-button">
            <img src="@/assets/Upload_89524_The_Noun_Project.png" alt="upload" class="upload-icon"/>
          </button>
        </td>
        <td class="actions-column" id="footer-row">
...
export default {
  ...
  methods: {
    ...
    async uploadFile(viewId, viewName, index) {
      ...
      try {
        const response = await axios.post(`https://vdi-api.nessi.no:9815/data/${viewName}`, formData, {
          headers: {
    ...
    async uploadFiles(viewId, viewName) {
      const formData = new FormData();
      console.log("ViewId: ", viewId)
      console.log("ViewName: ", viewName)
